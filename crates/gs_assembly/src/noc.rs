...
const NIU_SLV_POSTED_WR_REQ_STARTED: u32 = 0x1D;
const NIU_SLV_NONPOSTED_WR_REQ_STARTED: u32 = 0x1C;
const NIU_SLV_POSTED_WR_REQ_RECEIVED: u32 = 0x1B;
const NIU_SLV_NONPOSTED_WR_REQ_RECEIVED: u32 = 0x1A;
const NIU_SLV_POSTED_WR_DATA_WORD_RECEIVED: u32 = 0x19;
const NIU_SLV_NONPOSTED_WR_DATA_WORD_RECEIVED: u32 = 0x18;
const NIU_SLV_POSTED_ATOMIC_RECEIVED: u32 = 0x17;
const NIU_SLV_NONPOSTED_ATOMIC_RECEIVED: u32 = 0x16;
const NIU_SLV_RD_REQ_RECEIVED: u32 = 0x15;

const NIU_SLV_REQ_ACCEPTED: u32 = 0x14;
const NIU_SLV_RD_DATA_WORD_SENT: u32 = 0x13;
const NIU_SLV_RD_RESP_SENT: u32 = 0x12;
const NIU_SLV_WR_ACK_SENT: u32 = 0x11;
const NIU_SLV_ATOMIC_RESP_SENT: u32 = 0x10;
...
#[no_mangle]
static NOC_DEBUG: SyncUnsafeCell<NocValue> = SyncUnsafeCell::new(NocValue(0));
...
pub fn send_non_atomic_packet(
    ...
    unsafe {
        ...
        (&raw mut (*NOC_DEBUG.get()).0).write_volatile(0xc0de0000 | value);
    }
    loop {
    ...
    unsafe {
        let value = (&raw mut (*NOC_DEBUG.get()).0).read_volatile();
        (&raw mut (*NOC_DEBUG.get()).0).write_volatile(0xc1de0000 | value);
    }
...
pub fn noc_read(sel: NocCommandSel, remote: NocAddr, buf: &mut [u8], wait: bool) {
    ...
    unsafe {
        (&raw mut (*NOC_DEBUG.get()).0).write_volatile(0x1200);
    }
    ...
    unsafe {
        (&raw mut (*NOC_DEBUG.get()).0).write_volatile(0x1260);
    }

    if wait {
        while read_response_count(sel) < rd_cfg(sel, NOC_STATUS_REG!(NIU_MST_RD_REQ_STARTED)) {}
    }

    unsafe {
        (&raw mut (*NOC_DEBUG.get()).0).write_volatile(0x1261);
    }
}
...
pub fn noc_write(sel: NocCommandSel, remote: NocAddr, buf: &[u8], wait: bool) {
    ...
    unsafe {
        (&raw mut (*NOC_DEBUG.get()).0).write_volatile(0x1300);
    }
    ...
    unsafe {
        (&raw mut (*NOC_DEBUG.get()).0).write_volatile(0x1360);
    }
    ...
    unsafe {
        (&raw mut (*NOC_DEBUG.get()).0).write_volatile(0x1361);
    }
}
...
fn atomic_increment(sel: NocCommandSel, dest: NocAddr, incr: u32, wrap: u32, linked: bool) {
    ...
    wr_cfg(
        ...
        NOC_AT_INS!(NOC_AT_INS_INCR_GET)
            | NOC_AT_WRAP!(wrap)
            | NOC_AT_IND_32!(((dest.offset >> 2) & 0x3) as u32)
            | NOC_AT_IND_32_SRC!(0),
    );
...
pub fn atomic_read_and_increment(
    ...
    wr_cfg(
        ...
        NOC_AT_INS!(NOC_AT_INS_INCR_GET)
            | NOC_AT_WRAP!(wrap)
            | NOC_AT_IND_32!(((remote.offset >> 2) & 0x3) as u32)
            | NOC_AT_IND_32_SRC!(0),
    );
