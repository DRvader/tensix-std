...
// NIU MST Control Regs
const NOC_REG_SPACE_START_ADDR: u32 = 0xFF000000;
const NOC_REGS_START_ADDR: u32 = 0xFFB20000;

const NOC_CMD_BUF_OFFSET: u32 = 0x00000400;
const NOC_CMD_BUF_OFFSET_BIT: u32 = 10;
...
const NOC_REG_AT_DATA: u32 = 0x24;

const NOC_REG_CMD_CTRL: u32 = 0x28;
const NOC_REG_NODE_ID: u32 = 0x2C;
const NOC_REG_ENDPOINT_ID: u32 = 0x30;

const NOG_REG_NUM_MEM_PARITY_ERR: u32 = 0x40;
const NOC_REG_NUM_HEADER_1B_ERR: u32 = 0x44;
const NOC_REG_NUM_HEADER_2B_ERR: u32 = 0x48;
const NOC_REG_ECC_CTRL: u32 = 0x4C; // [2:0] = clear ECC interrupts, [5:3] = force ECC error

const NOC_CLEAR_OUTSTANDING_REQ_CNT: u32 = 0x50;
...
macro_rules! NOC_MST_WRITE_REQS_OUTGOING_ID {
    ($id:expr) => {
        ($reg as u32) + 0x20
    };
}

macro_rules! NOC_MST_REQS_OUTSTANDING_ID {
    ($id:expr) => {
        ($reg as u32) + 0x10
    };
}
...
const NOC_DATA_WIDTH: u16 = 259;
const NOC_PAYLOAD_WIDTH: u16 = 256;
...
const NOC_VCS: u32 = 16;
const NOC_BROADCAST_VC_START: u32 = 4;
const NOC_PORT_NIU: u32 = 0;
const NOC_PORT_X: u32 = 1;
const NOC_PORT_Y: u32 = 2;

#[repr(align(32))]
pub struct NocValue(u32);

#[no_mangle]
static NOC_DEBUG: core::cell::SyncUnsafeCell<NocValue> =
    core::cell::SyncUnsafeCell::new(NocValue(0));

fn debug_new_func(code: u8) {
    unsafe {
        (&raw mut (*NOC_DEBUG.get()).0).write_volatile((code as u32) << 24);
    }
}

fn debug_new_chunk(code: u8) {
    unsafe {
        let value = (&raw mut (*NOC_DEBUG.get()).0).read_volatile();
        (&raw mut (*NOC_DEBUG.get()).0)
            .write_volatile((value & 0xFF000000) | ((code as u32) << 16));
    }
}
...
fn debug_new_line() {
    unsafe {
        let value = (&raw mut (*NOC_DEBUG.get()).0).read_volatile();
        (&raw mut (*NOC_DEBUG.get()).0).write_volatile(value + 1);
    }
}

fn debug_set_line(line: u16) {
    unsafe {
        let value = (&raw mut (*NOC_DEBUG.get()).0).read_volatile();
        (&raw mut (*NOC_DEBUG.get()).0).write_volatile((value & 0xFFFF0000) | line as u32);
    }
}
...
impl NocAddr {
    ...
    pub fn upper_addr(&self) -> u32 {
        (((self.offset >> 32) & 0xF) as u32)
            | ((self.x_end as u32) << 4)
            | ((self.y_end as u32) << 10)
            | ((self.x_start as u32) << 16)
            | ((self.y_start as u32) << 22)
    }
...
pub fn send_non_atomic_packet(
    ...
    wr_cfg(sel, NOC_REG_TARG_ADDR_MID, src.upper_addr());

    wr_cfg(sel, NOC_REG_RET_ADDR_LO, dest.lower_addr());
    wr_cfg(sel, NOC_REG_RET_ADDR_MID, dest.upper_addr());
...
fn send_non_atomic_packets(
    ...
    if send_non_atomic_packet(sel, src, dest, LenOrBE::Len(size as u16), ctrl.clone()) {
        packets_sent += 1;
    }
...
pub fn noc_read(sel: NocCommandSel, remote: NocAddr, buf: &mut [u8], wait: bool) {
    ...
    let expected_responses = read_response_count(sel);
    let expected_responses = expected_responses
        + send_non_atomic_packets(
            sel,
            remote,
            local,
            buf.len() as u16,
    ...
    debug_new_chunk(0x2);
    debug_set_line(expected_responses as u16);

    if wait {
        while read_response_count(sel) < expected_responses {}
    }
...
pub fn noc_write(sel: NocCommandSel, remote: NocAddr, buf: &[u8], wait: bool) {
    ...
    let expected_acks = write_ack_count(sel);
    let expected_acks = expected_acks
        + send_non_atomic_packets(
            sel,
            local,
            remote,
    ...
    debug_new_chunk(0x2);
    debug_set_line(expected_acks as u16);

    if wait {
        while write_ack_count(sel) < expected_acks {}
    }
