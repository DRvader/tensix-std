...
macro_rules! APOOL3S1_value {
    ($clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
        { $crate::tt_op!(0x25, ($clear_dvalid << 22) | ($addr_mode << 15) | ($index_en << 14) | ($dst << 0)) }
    };
}
#[macro_export]
macro_rules! APOOL3S1_valid {
    ($clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
        $crate::asm::is_valid(clear_dvalid, 2) && $crate::asm::is_valid(addr_mode, 7) && $crate::asm::is_valid(index_en, 1) && $crate::asm::is_valid(dst, 14)
    };
}
#[macro_export]
macro_rules! APOOL3S1 {
    ($clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
    { $crate::instruction_word!($crate::trisc_op_swizzle!($crate::asm::APOOL3S1_value!($clear_dvalid, $addr_mode, $index_en, $dst))); }
    };
}
#[macro_export]
macro_rules! APOOL3S1d {
    ($clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
    { $crate::push_instrn($crate::asm::APOOL3S1_value!($clear_dvalid, $addr_mode, $index_en, $dst)); }
    };
}
#[macro_export]
macro_rules! brisc_APOOL3S1d {
    ($core:path, $clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
    { $crate::brisc_push_instrn::<{ $core }>($crate::asm::APOOL3S1_value!($clear_dvalid, $addr_mode, $index_en, $dst)); }
    };
}
...
macro_rules! APOOL3S2_value {
    ($clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
        { $crate::tt_op!(0x32, ($clear_dvalid << 22) | ($addr_mode << 15) | ($index_en << 14) | ($dst << 0)) }
    };
}
#[macro_export]
...
macro_rules! APOOL3S2 {
    ($clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
    { $crate::instruction_word!($crate::trisc_op_swizzle!($crate::asm::APOOL3S2_value!($clear_dvalid, $addr_mode, $index_en, $dst))); }
    };
}
#[macro_export]
macro_rules! APOOL3S2d {
    ($clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
    { $crate::push_instrn($crate::asm::APOOL3S2_value!($clear_dvalid, $addr_mode, $index_en, $dst)); }
    };
}
#[macro_export]
macro_rules! brisc_APOOL3S2d {
    ($core:path, $clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
    { $crate::brisc_push_instrn::<{ $core }>($crate::asm::APOOL3S2_value!($clear_dvalid, $addr_mode, $index_en, $dst)); }
    };
}
...
macro_rules! DOTPV_value {
    ($clear_dvalid:expr, $dest_accum_en:expr, $instr_mod19:expr, $addr_mode:expr, $dst:expr) => {
        { $crate::tt_op!(0x29, ($clear_dvalid << 22) | ($dest_accum_en << 21) | ($instr_mod19 << 19) | ($addr_mode << 15) | ($dst << 0)) }
    };
}
#[macro_export]
macro_rules! DOTPV_valid {
    ($clear_dvalid:expr, $dest_accum_en:expr, $instr_mod19:expr, $addr_mode:expr, $dst:expr) => {
        $crate::asm::is_valid(clear_dvalid, 2) && $crate::asm::is_valid(dest_accum_en, 1) && $crate::asm::is_valid(instr_mod19, 2) && $crate::asm::is_valid(addr_mode, 4) && $crate::asm::is_valid(dst, 15)
    };
}
#[macro_export]
macro_rules! DOTPV {
...
macro_rules! ELWADD_value {
    ($clear_dvalid:expr, $dest_accum_en:expr, $instr_mod19:expr, $addr_mode:expr, $dst:expr) => {
        { $crate::tt_op!(0x28, ($clear_dvalid << 22) | ($dest_accum_en << 21) | ($instr_mod19 << 19) | ($addr_mode << 15) | ($dst << 0)) }
    };
}
#[macro_export]
...
macro_rules! ELWMUL_value {
    ($clear_dvalid:expr, $dest_accum_en:expr, $instr_mod19:expr, $addr_mode:expr, $dst:expr) => {
        { $crate::tt_op!(0x27, ($clear_dvalid << 22) | ($dest_accum_en << 21) | ($instr_mod19 << 19) | ($addr_mode << 15) | ($dst << 0)) }
    };
}
#[macro_export]
...
macro_rules! ELWSUB_value {
    ($clear_dvalid:expr, $dest_accum_en:expr, $instr_mod19:expr, $addr_mode:expr, $dst:expr) => {
        { $crate::tt_op!(0x30, ($clear_dvalid << 22) | ($dest_accum_en << 21) | ($instr_mod19 << 19) | ($addr_mode << 15) | ($dst << 0)) }
    };
}
#[macro_export]
...
macro_rules! GAPOOL_value {
    ($clear_dvalid:expr, $instr_mod19:expr, $addr_mode:expr, $max_pool_index_en:expr, $dst:expr) => {
        { $crate::tt_op!(0x34, ($clear_dvalid << 22) | ($instr_mod19 << 19) | ($addr_mode << 15) | ($max_pool_index_en << 14) | ($dst << 0)) }
    };
}
#[macro_export]
macro_rules! GAPOOL_valid {
    ($clear_dvalid:expr, $instr_mod19:expr, $addr_mode:expr, $max_pool_index_en:expr, $dst:expr) => {
        $crate::asm::is_valid(clear_dvalid, 2) && $crate::asm::is_valid(instr_mod19, 3) && $crate::asm::is_valid(addr_mode, 4) && $crate::asm::is_valid(max_pool_index_en, 1) && $crate::asm::is_valid(dst, 14)
    };
}
#[macro_export]
macro_rules! GAPOOL {
    ($clear_dvalid:expr, $instr_mod19:expr, $addr_mode:expr, $max_pool_index_en:expr, $dst:expr) => {
    { $crate::instruction_word!($crate::trisc_op_swizzle!($crate::asm::GAPOOL_value!($clear_dvalid, $instr_mod19, $addr_mode, $max_pool_index_en, $dst))); }
    };
}
#[macro_export]
macro_rules! GAPOOLd {
    ($clear_dvalid:expr, $instr_mod19:expr, $addr_mode:expr, $max_pool_index_en:expr, $dst:expr) => {
    { $crate::push_instrn($crate::asm::GAPOOL_value!($clear_dvalid, $instr_mod19, $addr_mode, $max_pool_index_en, $dst)); }
    };
}
#[macro_export]
macro_rules! brisc_GAPOOLd {
    ($core:path, $clear_dvalid:expr, $instr_mod19:expr, $addr_mode:expr, $max_pool_index_en:expr, $dst:expr) => {
    { $crate::brisc_push_instrn::<{ $core }>($crate::asm::GAPOOL_value!($clear_dvalid, $instr_mod19, $addr_mode, $max_pool_index_en, $dst)); }
    };
}
...
macro_rules! GMPOOL_value {
    ($clear_dvalid:expr, $instr_mod19:expr, $addr_mode:expr, $max_pool_index_en:expr, $dst:expr) => {
        { $crate::tt_op!(0x33, ($clear_dvalid << 22) | ($instr_mod19 << 19) | ($addr_mode << 15) | ($max_pool_index_en << 14) | ($dst << 0)) }
    };
}
#[macro_export]
...
macro_rules! GMPOOL {
    ($clear_dvalid:expr, $instr_mod19:expr, $addr_mode:expr, $max_pool_index_en:expr, $dst:expr) => {
    { $crate::instruction_word!($crate::trisc_op_swizzle!($crate::asm::GMPOOL_value!($clear_dvalid, $instr_mod19, $addr_mode, $max_pool_index_en, $dst))); }
    };
}
#[macro_export]
macro_rules! GMPOOLd {
    ($clear_dvalid:expr, $instr_mod19:expr, $addr_mode:expr, $max_pool_index_en:expr, $dst:expr) => {
    { $crate::push_instrn($crate::asm::GMPOOL_value!($clear_dvalid, $instr_mod19, $addr_mode, $max_pool_index_en, $dst)); }
    };
}
#[macro_export]
macro_rules! brisc_GMPOOLd {
    ($core:path, $clear_dvalid:expr, $instr_mod19:expr, $addr_mode:expr, $max_pool_index_en:expr, $dst:expr) => {
    { $crate::brisc_push_instrn::<{ $core }>($crate::asm::GMPOOL_value!($clear_dvalid, $instr_mod19, $addr_mode, $max_pool_index_en, $dst)); }
    };
}
...
macro_rules! MFCONV3S1_value {
    ($clear_dvalid:expr, $rotate_weights:expr, $addr_mode:expr, $dst:expr) => {
        { $crate::tt_op!(0x3a, ($clear_dvalid << 22) | ($rotate_weights << 17) | ($addr_mode << 15) | ($dst << 0)) }
    };
}
#[macro_export]
...
macro_rules! MOVA2D_value {
    ($dest_32b_lo:expr, $src:expr, $addr_mode:expr, $instr_mod:expr, $dst:expr) => {
        { $crate::tt_op!(0x12, ($dest_32b_lo << 23) | ($src << 17) | ($addr_mode << 15) | ($instr_mod << 12) | ($dst << 0)) }
    };
}
#[macro_export]
macro_rules! MOVA2D_valid {
    ($dest_32b_lo:expr, $src:expr, $addr_mode:expr, $instr_mod:expr, $dst:expr) => {
        $crate::asm::is_valid(dest_32b_lo, 1) && $crate::asm::is_valid(src, 6) && $crate::asm::is_valid(addr_mode, 2) && $crate::asm::is_valid(instr_mod, 3) && $crate::asm::is_valid(dst, 12)
    };
}
#[macro_export]
macro_rules! MOVA2D {
...
macro_rules! MOVB2A_value {
    ($srca:expr, $addr_mode:expr, $instr_mod:expr, $srcb:expr) => {
        { $crate::tt_op!(0x0b, ($srca << 17) | ($addr_mode << 15) | ($instr_mod << 12) | ($srcb << 0)) }
    };
}
#[macro_export]
macro_rules! MOVB2A_valid {
    ($srca:expr, $addr_mode:expr, $instr_mod:expr, $srcb:expr) => {
        $crate::asm::is_valid(srca, 7) && $crate::asm::is_valid(addr_mode, 2) && $crate::asm::is_valid(instr_mod, 3) && $crate::asm::is_valid(srcb, 12)
    };
}
#[macro_export]
macro_rules! MOVB2A {
...
macro_rules! MOVB2D_value {
    ($dest_32b_lo:expr, $src:expr, $addr_mode:expr, $instr_mod:expr, $dst:expr) => {
        { $crate::tt_op!(0x13, ($dest_32b_lo << 23) | ($src << 17) | ($addr_mode << 15) | ($instr_mod << 12) | ($dst << 0)) }
    };
}
#[macro_export]
...
macro_rules! MOVB2D {
    ($dest_32b_lo:expr, $src:expr, $addr_mode:expr, $instr_mod:expr, $dst:expr) => {
    { $crate::instruction_word!($crate::trisc_op_swizzle!($crate::asm::MOVB2D_value!($dest_32b_lo, $src, $addr_mode, $instr_mod, $dst))); }
    };
}
#[macro_export]
macro_rules! MOVB2Dd {
    ($dest_32b_lo:expr, $src:expr, $addr_mode:expr, $instr_mod:expr, $dst:expr) => {
    { $crate::push_instrn($crate::asm::MOVB2D_value!($dest_32b_lo, $src, $addr_mode, $instr_mod, $dst)); }
    };
}
#[macro_export]
macro_rules! brisc_MOVB2Dd {
    ($core:path, $dest_32b_lo:expr, $src:expr, $addr_mode:expr, $instr_mod:expr, $dst:expr) => {
    { $crate::brisc_push_instrn::<{ $core }>($crate::asm::MOVB2D_value!($dest_32b_lo, $src, $addr_mode, $instr_mod, $dst)); }
    };
}
...
macro_rules! MOVD2A_value {
    ($dest_32b_lo:expr, $src:expr, $addr_mode:expr, $instr_mod:expr, $dst:expr) => {
        { $crate::tt_op!(0x08, ($dest_32b_lo << 23) | ($src << 17) | ($addr_mode << 15) | ($instr_mod << 12) | ($dst << 0)) }
    };
}
#[macro_export]
...
macro_rules! MOVD2B_value {
    ($dest_32b_lo:expr, $src:expr, $addr_mode:expr, $instr_mod:expr, $dst:expr) => {
        { $crate::tt_op!(0x0a, ($dest_32b_lo << 23) | ($src << 17) | ($addr_mode << 15) | ($instr_mod << 12) | ($dst << 0)) }
    };
}
#[macro_export]
...
macro_rules! MOVDBGA2D_value {
    ($dest_32b_lo:expr, $src:expr, $addr_mode:expr, $instr_mod:expr, $dst:expr) => {
        { $crate::tt_op!(0x09, ($dest_32b_lo << 23) | ($src << 17) | ($addr_mode << 15) | ($instr_mod << 12) | ($dst << 0)) }
    };
}
#[macro_export]
...
macro_rules! MPOOL3S1_value {
    ($clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
        { $crate::tt_op!(0x24, ($clear_dvalid << 22) | ($addr_mode << 15) | ($index_en << 14) | ($dst << 0)) }
    };
}
#[macro_export]
...
macro_rules! MPOOL3S1 {
    ($clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
    { $crate::instruction_word!($crate::trisc_op_swizzle!($crate::asm::MPOOL3S1_value!($clear_dvalid, $addr_mode, $index_en, $dst))); }
    };
}
#[macro_export]
macro_rules! MPOOL3S1d {
    ($clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
    { $crate::push_instrn($crate::asm::MPOOL3S1_value!($clear_dvalid, $addr_mode, $index_en, $dst)); }
    };
}
#[macro_export]
macro_rules! brisc_MPOOL3S1d {
    ($core:path, $clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
    { $crate::brisc_push_instrn::<{ $core }>($crate::asm::MPOOL3S1_value!($clear_dvalid, $addr_mode, $index_en, $dst)); }
    };
}
...
macro_rules! MPOOL3S2_value {
    ($clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
        { $crate::tt_op!(0x31, ($clear_dvalid << 22) | ($addr_mode << 15) | ($index_en << 14) | ($dst << 0)) }
    };
}
#[macro_export]
...
macro_rules! MPOOL3S2 {
    ($clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
    { $crate::instruction_word!($crate::trisc_op_swizzle!($crate::asm::MPOOL3S2_value!($clear_dvalid, $addr_mode, $index_en, $dst))); }
    };
}
#[macro_export]
macro_rules! MPOOL3S2d {
    ($clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
    { $crate::push_instrn($crate::asm::MPOOL3S2_value!($clear_dvalid, $addr_mode, $index_en, $dst)); }
    };
}
#[macro_export]
macro_rules! brisc_MPOOL3S2d {
    ($core:path, $clear_dvalid:expr, $addr_mode:expr, $index_en:expr, $dst:expr) => {
    { $crate::brisc_push_instrn::<{ $core }>($crate::asm::MPOOL3S2_value!($clear_dvalid, $addr_mode, $index_en, $dst)); }
    };
}
...
macro_rules! MVMUL_value {
    ($clear_dvalid:expr, $instr_mod19:expr, $addr_mode:expr, $dst:expr) => {
        { $crate::tt_op!(0x26, ($clear_dvalid << 22) | ($instr_mod19 << 19) | ($addr_mode << 15) | ($dst << 0)) }
    };
}
#[macro_export]
macro_rules! MVMUL_valid {
    ($clear_dvalid:expr, $instr_mod19:expr, $addr_mode:expr, $dst:expr) => {
        $crate::asm::is_valid(clear_dvalid, 2) && $crate::asm::is_valid(instr_mod19, 3) && $crate::asm::is_valid(addr_mode, 4) && $crate::asm::is_valid(dst, 15)
    };
}
#[macro_export]
macro_rules! MVMUL {
...
macro_rules! PACR_SETREG_value {
    ($Push:expr, $AddrSel:expr, $WrData:expr, $PackSel:expr, $StreamId:expr, $Flush:expr, $Last:expr) => {
        { $crate::tt_op!(0x4a, ($Push << 23) | ($AddrSel << 22) | ($WrData << 12) | ($PackSel << 8) | ($StreamId << 2) | ($Flush << 1) | ($Last << 0)) }
    };
}
#[macro_export]
macro_rules! PACR_SETREG_valid {
    ($Push:expr, $AddrSel:expr, $WrData:expr, $PackSel:expr, $StreamId:expr, $Flush:expr, $Last:expr) => {
        $crate::asm::is_valid(Push, 1) && $crate::asm::is_valid(AddrSel, 1) && $crate::asm::is_valid(WrData, 10) && $crate::asm::is_valid(PackSel, 4) && $crate::asm::is_valid(StreamId, 6) && $crate::asm::is_valid(Flush, 1) && $crate::asm::is_valid(Last, 1)
    };
}
#[macro_export]
macro_rules! PACR_SETREG {
    ($Push:expr, $AddrSel:expr, $WrData:expr, $PackSel:expr, $StreamId:expr, $Flush:expr, $Last:expr) => {
    { $crate::instruction_word!($crate::trisc_op_swizzle!($crate::asm::PACR_SETREG_value!($Push, $AddrSel, $WrData, $PackSel, $StreamId, $Flush, $Last))); }
    };
}
#[macro_export]
macro_rules! PACR_SETREGd {
    ($Push:expr, $AddrSel:expr, $WrData:expr, $PackSel:expr, $StreamId:expr, $Flush:expr, $Last:expr) => {
    { $crate::push_instrn($crate::asm::PACR_SETREG_value!($Push, $AddrSel, $WrData, $PackSel, $StreamId, $Flush, $Last)); }
    };
}
#[macro_export]
macro_rules! brisc_PACR_SETREGd {
    ($core:path, $Push:expr, $AddrSel:expr, $WrData:expr, $PackSel:expr, $StreamId:expr, $Flush:expr, $Last:expr) => {
    { $crate::brisc_push_instrn::<{ $core }>($crate::asm::PACR_SETREG_value!($Push, $AddrSel, $WrData, $PackSel, $StreamId, $Flush, $Last)); }
    };
}
...
macro_rules! SFPLOAD_value {
    ($lreg_ind:expr, $instr_mod0:expr, $sfpu_addr_mode:expr, $dest_reg_addr:expr) => {
        { $crate::tt_op!(0x70, ($lreg_ind << 20) | ($instr_mod0 << 16) | ($sfpu_addr_mode << 14) | ($dest_reg_addr << 0)) }
    };
}
#[macro_export]
macro_rules! SFPLOAD_valid {
    ($lreg_ind:expr, $instr_mod0:expr, $sfpu_addr_mode:expr, $dest_reg_addr:expr) => {
        $crate::asm::is_valid(lreg_ind, 4) && $crate::asm::is_valid(instr_mod0, 4) && $crate::asm::is_valid(sfpu_addr_mode, 2) && $crate::asm::is_valid(dest_reg_addr, 14)
    };
}
#[macro_export]
macro_rules! SFPLOAD {
...
macro_rules! SFPLOADMACRO_value {
    ($lreg_ind:expr, $instr_mod0:expr, $sfpu_addr_mode:expr, $dest_reg_addr:expr) => {
        { $crate::tt_op!(0x93, ($lreg_ind << 20) | ($instr_mod0 << 16) | ($sfpu_addr_mode << 14) | ($dest_reg_addr << 0)) }
    };
}
#[macro_export]
...
macro_rules! SFPSTORE_value {
    ($lreg_ind:expr, $instr_mod0:expr, $sfpu_addr_mode:expr, $dest_reg_addr:expr) => {
        { $crate::tt_op!(0x72, ($lreg_ind << 20) | ($instr_mod0 << 16) | ($sfpu_addr_mode << 14) | ($dest_reg_addr << 0)) }
    };
}
#[macro_export]
...
macro_rules! SHIFTXB_value {
    ($addr_mode:expr, $rot_shift:expr, $shift_row:expr) => {
        { $crate::tt_op!(0x18, ($addr_mode << 15) | ($rot_shift << 10) | ($shift_row << 0)) }
    };
}
#[macro_export]
macro_rules! SHIFTXB_valid {
    ($addr_mode:expr, $rot_shift:expr, $shift_row:expr) => {
        $crate::asm::is_valid(addr_mode, 9) && $crate::asm::is_valid(rot_shift, 5) && $crate::asm::is_valid(shift_row, 10)
    };
}
#[macro_export]
macro_rules! SHIFTXB {
