use crate::{
    ...
    cfg_defs::{
        ...
        THCON_SEC1_REG0_TileDescriptor_ADDR32, UNP0_ADDR_CTRL_ZW_REG_1_Zstride_ADDR32,
    },
    tensix_defs::REGFILE_BASE,
    Core, DataFormat, SETADCXX_value, SETADCXY_value, SETADCXYd, SETADCZW_value, SETADCZWd,
    SETC16d, SETDMAREGd, WRCFGd,
};
...
impl UnpackSettings {
    ...
    fn pack_tile_descriptor(&self) -> (u32, u32, u32, u32) {
        (
            ((self.x_dim as u32) << 16) | ((self.compressed as u32) << 4) | (self.l1_format as u32),
...
pub fn configure(srca: UnpackSettings, srcb: UnpackSettings) {
    ...
    let (real_srca_fmt, _tile_offset) =
        hw_workarounds::weird_unpacr_workarounds(srcb.l1_format, srcb.reg_format);
    cfg_rmw!(THCON_SEC1_REG2_Out_data_format, real_srca_fmt as u32);
    ...
    asm::SETADCXXd!(
        BlockSel::UNPCKER0.0,
        srca.x_dim as u32 * hw_workarounds::tile_size(srca.l1_format, srca.x_dim),
        0
    );
    asm::SETADCXXd!(
        BlockSel::UNPCKER1.0,
        srcb.x_dim as u32 * hw_workarounds::tile_size(srcb.l1_format, srcb.x_dim),
        0
    );
}
